...
const calendarUtil = { 
    getMonths:function(y,m){
      const tempArray=LunarCalendar.calendar(y,m,true);
      const {monthData=[]}=tempArray,backArray=[];
      let temp=[];
      monthData.map((day,i)=>{
        day.week=moment(day.year+'-'+day.month+'-'+day.day).format('d'); 
        if(i % 7==0){   
          temp=[day];
        }else if(i % 7==6){
          temp.push(day);
    ...
    getMonthsStartEnd:function(y,m){
      const tempArray=LunarCalendar.calendar(y,m,true);
      const {monthData=[]}=tempArray;
      const start = monthData.shift();
      const end = monthData.pop();
      const startTime = start.year+"-"+start.month+"-"+start.day+" 00:00:01";
      const endTime   = end.year+"-"+end.month+"-"+end.day+" 23:59:59";
      return [
        startTime,
        endTime
      ]
    },
    getSolarToLunar:function(currentDay){
      return LunarCalendar.solarToLunar(currentDay.format('YYYY'),currentDay.format('MM'),currentDay.format('DD'))
    }
};
